...
export function ProfileComponent() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      if (user) {
        // Get name directly from user metadata
        ...
        // Fetch user's story and other data
        ...
        // Fetch user's routines separately
  ...
  const createWeeklyTasks = (startDate: Date) => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      if (routines) {
        ...
        parsedRoutines.dailyRoutines.forEach((routine: string, index: number) => {
          weeklyTasks.push({
      ...
      <div className="w-full max-w-[100vw] mx-auto px-4 py-4 sm:max-w-md sm:py-8 sm:px-4">
        ...
        <div className="flex justify-center mb-8">
          <ProgressRing
            ...
            avatarFallback={
              <Avatar>
                <AvatarFallback>Upload image here</AvatarFallback>
              </Avatar>
            }
          />
        </div>
        ...
        <TaskList 
          initialTasks={tasks.filter(task => task.date === selectedDate.toISOString().split('T')[0])} 
